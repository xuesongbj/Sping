...
/* 设置的时间是一个结构体，倒计时设置，重复倒时，超时值设为1s */
...
void send_ping(void)
{
	...
	ip_hdr->hlen = sizeof(struct iphdr) >> 2;	/* 头部长度; 5*4=20byte */
